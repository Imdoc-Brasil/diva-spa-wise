...
const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  }, [isOpen, selectedIndex, query]); // Added query dependency
  ...
  const results: SearchResult[] = [
    ...
    // --- MOCK CLIENTS ---
    { id: 'cli-1', title: 'Ana Silva', subtitle: 'Cliente VIP • Última visita: Ontem', type: 'client', icon: <User size={18} />, action: () => navigate('/crm') },
    { id: 'cli-2', title: 'Beatriz Costa', subtitle: 'Novo Lead', type: 'client', icon: <User size={18} />, action: () => navigate('/crm') },
  ];
                    ...
                    {filteredResults.map((result, index) => (
                        <div
                            ...
                            className={`flex items-center justify-between px-4 py-3 rounded-lg cursor-pointer transition-colors ${
                                index === selectedIndex ? 'bg-diva-primary text-white' : 'text-gray-700 hover:bg-gray-50'
                            }`}
                        >
