...
import { Filter, Plus, MoreHorizontal, MessageCircle, Instagram, Globe, Users, TrendingUp, X, Save, UserPlus, Search, Phone, Sparkles } from 'lucide-react';
...
const FunnelModule: React.FC = () => {
    const { leads, addLead, updateLeadStage, updateLead, removeLead, clients } = useData();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingLead, setEditingLead] = useState<SalesLead | null>(null);
    const [draggingId, setDraggingId] = useState<string | null>(null);
    const [automationsRun, setAutomationsRun] = useState(false);
    const [automationResults, setAutomationResults] = useState({ moved: 0, contacted: 0 });

    const handleRunAutomations = () => {
        // Real automation logic: move leads that are stagnant for more than 24h
        const now = new Date();
        let movedCount = 0;
        ...
        leads.forEach(lead => {
            ...
            // Rule 1: Leads in "NEW" for more than 24h should be contacted
            if (lead.stage === LeadStage.NEW && hoursSinceActivity > 24) {
                updateLeadStage(lead.leadId, LeadStage.CONTACTED);
                movedCount++;
                contactedCount++;
            }

            // Rule 2: Leads in "CONTACTED" for more than 48h with no scheduled appointment should be flagged
            if (lead.stage === LeadStage.CONTACTED && hoursSinceActivity > 48) {
                // In a real app, this would create a task or send a notification
                console.log(`Lead ${lead.name} needs follow-up`);
            }
        });

        setAutomationResults({ moved: movedCount, contacted: contactedCount });
        setAutomationsRun(true);
        setTimeout(() => setAutomationsRun(false), 8000);
    };
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)]">
            ...
            <div className="flex-1 flex flex-col min-w-0">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="bg-white border border-diva-light/40 text-diva-dark px-3 py-2 rounded-lg text-sm font-medium flex items-center hover:bg-gray-50 transition-colors shadow-sm"
                            title="Verificar leads estagnados e mover automaticamente"
                        >
                            <Sparkles size={16} className="mr-2 text-diva-accent" /> Automação
                        </button>
                        <button
                ...
                {automationsRun && (
                    <div className="mb-4 bg-diva-accent/10 border border-diva-accent/20 p-3 rounded-lg flex justify-between items-center animate-in fade-in slide-in-from-top-2">
                        <div className="flex items-center text-sm text-diva-dark">
                            <Sparkles size={16} className="mr-2 text-diva-accent" />
                            <span>
                                <strong>Automação Executada:</strong> {automationResults.moved > 0
                                    ? `${automationResults.moved} lead(s) estagnado(s) foram movidos para "Em Contato" e receberão mensagem automática.`
                                    : 'Nenhum lead estagnado detectado. Todos os leads estão dentro do prazo esperado!'}
                            </span>
                        </div>
                        <button onClick={() => setAutomationsRun(false)} className="text-gray-400 hover:text-diva-dark"><X size={16} /></button>
                    </div>
                )}
            ...
            <LeadModal
                ...
                onDelete={() => editingLead && removeLead(editingLead.leadId)}
