...
import { Product, ProductCategory, Supplier, PurchaseOrder, StockAudit, StockAuditItem, User, UserRole } from '../../types';
import { ShoppingBag, Search, Tag, Heart, Plus, Minus, CreditCard, Sparkles, Package, AlertTriangle, TrendingDown, ArrowUpRight, ArrowDownRight, Archive, Truck, ClipboardList, CheckCircle, User as UserIcon, ClipboardCheck, Save, BarChart2, TrendingUp, Calendar } from 'lucide-react';
...
const MarketplaceModule: React.FC<MarketplaceModuleProps> = ({ user }) => {
    ...
    const getProductStock = (product: Product): number => {
        if (selectedUnitId === 'all') {
            ...
            if (product.stockByUnit) {
                total += Object.values(product.stockByUnit).reduce((a, b) => a + b, 0);
            }
            return total;
        }
        if (product.stockByUnit && product.stockByUnit[selectedUnitId] !== undefined) {
            return product.stockByUnit[selectedUnitId];
        }
    ...
    const getStockStatus = (prod: Product) => {
        ...
        if (prod.minStockLevel && stock <= prod.minStockLevel) return 'low';
        return 'good';
    };
    ...
    // Audit Logic
    const startAudit = () => {
        const auditItems: StockAuditItem[] = products.filter(p => getProductStock(p) !== undefined).map(p => ({
            productId: p.id,
            productName: p.name,
                            ...
                            <div className="flex space-x-2 overflow-x-auto pb-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${activeCategory === cat.id
                    ...
                    <div className="w-full lg:w-80 bg-white rounded-xl shadow-lg border border-diva-light/30 flex flex-col h-full">
                        ...
                        <div className="p-5 border-t border-diva-light/30 bg-gray-50 rounded-b-xl">
                            ...
                            <button
                                ...
                                className={`w-full text-white py-3 rounded-lg font-bold flex items-center justify-center transition-colors ${cart.length === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-diva-primary hover:bg-diva-dark'}`}
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 border-b border-diva-light/20 bg-gray-50">
                        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <p className="text-xl font-bold text-diva-dark mt-1">
                                {formatCurrency(products.reduce((acc, p) => acc + (getProductStock(p) * (p.costPrice || 0)), 0))}
                            </p>
                        </div>
                        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <p className="text-xl font-bold text-diva-alert mt-1 flex items-center">
                                {products.filter(p => getStockStatus(p) === 'low' || getStockStatus(p) === 'critical').length}
                        ...
                        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <p className="text-xl font-bold text-orange-500 mt-1">
                                {products.filter(p => isNearExpiry(p.expirationDate)).length}
                            </p>
                        </div>
                    ...
                    <div className="flex-1 overflow-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100 text-sm">
                                {products.filter(p => getProductStock(p) !== undefined).map(prod => {
