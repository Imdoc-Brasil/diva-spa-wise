...
const mockOpenVials: OpenVial[] = [
    ...
    { id: 'v3', productId: 'prod_nctf', productName: 'NCTF 135HA', batchNumber: 'NC-9900', openedAt: new Date(Date.now() - 1 * 60 * 60 * 1000).toISOString(), expiresAfterOpen: 12, initialUnits: 3, remainingUnits: 1.5, openedBy: 'Enf. Mariana' },
];

const mockUsageLogs: VialUsageLog[] = [
    ...
    { id: 'u2', vialId: 'v2', productName: 'Dysport', unitsUsed: 180, patientName: 'Beatriz Costa', procedure: 'Hiperidrose Axilar', timestamp: new Date(Date.now() - 23 * 60 * 60 * 1000).toISOString(), staffName: 'Dra. Julia' },
];

const PharmacyModule: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'fridge' | 'calculator' | 'traceability'>('fridge');
  const [vials, setVials] = useState(mockOpenVials);
