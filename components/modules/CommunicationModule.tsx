...
import { ChatConversation, ChannelType } from '../../types';
import { Search, Send, Paperclip, MessageCircle, Instagram, Mail, MoreVertical, Check, User, Clock, Calendar, DollarSign, Smile, Video } from 'lucide-react';
...
const mockConversations: ChatConversation[] = [
  ...
  },
  {
    ...
    channel: 'email',
    lastMessage: 'Confirmação de Agendamento',
    lastMessageTime: '15/10',
    unreadCount: 0,
    status: 'open',
    messages: [
        { id: 'm9', content: 'Confirmação de Agendamento: Depilação a Laser - 20/10 às 14h', sender: 'system', timestamp: '09:00', read: true },
    ]
  }
];
...
const CommunicationModule: React.FC = () => {
  ...
  const [selectedChatId, setSelectedChatId] = useState<string | null>(mockConversations[0].id);
  ...
  const selectedChat = mockConversations.find(c => c.id === selectedChatId);
