...
const Dashboard: React.FC = () => {
  ...
  const { transactions, appointments, leads, clients } = useData();
                        ...
                        <div key={appt.appointmentId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100 transition-colors hover:border-diva-primary cursor-pointer" onClick={() => navigate('/schedule')}>
                            ...
                            <div className="flex items-center">
                               <span className={`text-[10px] px-2 py-1 rounded font-bold uppercase border ${
                                   appt.status === AppointmentStatus.CONFIRMED ? 'bg-green-100 text-green-700 border-green-200' : 'bg-blue-100 text-blue-700 border-blue-200'
                               }`}>
                                   {appt.status}
                               </span>
                            </div>
