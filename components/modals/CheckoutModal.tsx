...
import { X, ShoppingBag, CreditCard, DollarSign, Smartphone, Plus, Trash2, Printer, CheckCircle, Ticket, Share2, Percent, Tag, Star } from 'lucide-react';
    ...
    const handlePayment = () => {
        ...
        setTimeout(() => {
            ...
            if (useLoyaltyPoints && client) {
                // Deduct all points since we used the full value (or up to total price)
                // Simplified: Deduct all points for now as we applied full value discount
                // Ideally calculate exactly how many points were needed if total < discount
            ...
            addToast('Venda registrada e estoque atualizado!', 'success');
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-diva-dark/80 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col md:flex-row h-[85vh]">
                ...
                <div className="flex-1 p-6 bg-gray-50 flex flex-col border-r border-gray-200 overflow-y-auto custom-scrollbar">
                    ...
                    {client && (client.loyaltyPoints || 0) > 0 && (
                        <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center justify-between animate-in slide-in-from-top-2 fade-in">
                            <div className="flex items-center gap-3">
                                <div className="bg-yellow-100 p-2 rounded-full text-yellow-600">
                                    <Star size={20} fill="currentColor" />
                                </div>
                                <div>
                                    <p className="text-sm font-bold text-yellow-800">Saldo de Pontos: {client.loyaltyPoints}</p>
                                    <p className="text-xs text-yellow-600">Vale {formatCurrency(loyaltyDiscountValue)} de desconto.</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setUseLoyaltyPoints(!useLoyaltyPoints)}
                                className={`text-xs px-3 py-1.5 rounded-lg font-bold transition-colors ${useLoyaltyPoints ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-yellow-500 text-white hover:bg-yellow-600'}`}
                            >
                                {useLoyaltyPoints ? 'Remover' : 'Usar Pontos'}
                            </button>
                        </div>
                    )}
